...
Route::get('contactusEmail', function(User $user, Request $request) {
    return view('emails.contactus', compact('user', 'orders'));
});

Route::get('testbranch', function (User $user, Order $orders) {
    $users = User::all();
    $orders = Auth::user()->orders;
    $hashedPassword = bcrypt('BCGD-2018');
//dd($hashedPassword);
...
Route::get('processBrochure', 'CategoryController@showProcessBrochureCategories')->name('processBrochure');
